...
# Assume input_lambda and ttt are defined
...
# Build data frame of true values
...
# Assume posterior samples data frame contains lambda1, lambda2, lambda3, pi1, pi2, pi3
posterior_df <- as.data.frame(target_est[1:samples_to_get, 1:6])  # example
...
# Convert to long format and tag parameter type
df_long <- df %>%
  pivot_longer(cols = everything(),
               names_to = "parameter",
               values_to = "value") %>%
...
# Build true-value data frame
true_vals_df <- data.frame(
  parameter = factor(c(lambda_names, pi_names), levels = c(lambda_names, pi_names)),
  true_value = c(lambda_true_pre, pi_true_pre)
)

# Plot lambda/pi separately, combine, and add true-value lines
p <- ggplot() +
  geom_density(

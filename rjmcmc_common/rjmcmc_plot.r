...
plot_trace_and_density <- function(est_k3, burn.in, iter,
  ...
  if (!is.null(truth) && length(truth) > 0) {
    for (v in truth) {
      p_trace <- p_trace + ggplot2::geom_hline(yintercept = v, linetype = "dashed",
  ...
  if (!is.null(truth) && length(truth) > 0) {
    for (v in truth) {
      p_density <- p_density + ggplot2::geom_vline(xintercept = v, linetype = "dashed",
                                                    color = "black", linewidth = 0.4)
    }
  }
  print(p_density)
...
plot_sixpanel_trace_density <- function(est_k3, burn.in, iter,
  ...
  build_param_df <- function(param) {
    if (param == "mu") {
      df <- post |>
        ...
        dplyr::mutate(comp = as.integer(sub("mu", "", name)),
                      param = "mu")
    } else if (param == "tau") {
      df <- post |>
        dplyr::select(dplyr::all_of(paste0("tau", 1:k))) |>
        dplyr::mutate(iter = seq_len(nrow(post))) |>
        tidyr::pivot_longer(cols = starts_with("tau"),
                            names_to = "name", values_to = "value") |>
        dplyr::mutate(comp = as.integer(sub("tau", "", name)),
                      param = "tau")
    } else { # "pi"
      ...
      df <- post |>
        ...
        dplyr::mutate(iter = seq_len(nrow(post))) |>
        tidyr::pivot_longer(cols = starts_with("pi"),
                            names_to = "name", values_to = "value") |>
        dplyr::mutate(comp = as.integer(sub("pi", "", name)),
                      param = "pi")
    }
    df |>
  ...
  add_truth_hlines <- function(p, truth_vec) {
    if (!is.null(truth_vec) && length(truth_vec) > 0) {
      for (val in truth_vec) {
        if (!is.na(val)) {
          p <- p + geom_hline(yintercept = val, linetype = "dashed",
  ...
  add_truth_vlines <- function(p, truth_vec) {
    if (!is.null(truth_vec) && length(truth_vec) > 0) {
      for (val in truth_vec) {
        if (!is.na(val)) {
          p <- p + geom_vline(xintercept = val, linetype = "dashed",
                              colour = "black", linewidth = 0.4)
        }
      }
    }
  ...
  make_trace <- function(df, y_symbol, truth_vec = NULL) {
  ...
  make_dens <- function(df, x_symbol, truth_vec = NULL) {
...
plot_rjmcmc_sixpanel <- function(mats,
  ...
  n_mu  <- nrow(Xmu); n_tau <- nrow(Xtau); n_pi  <- nrow(Xpi)
  ...
  add_truth_hlines <- function(p, truth_vec) {
    if (!is.null(truth_vec) && length(truth_vec) > 0) {
      for (val in truth_vec) if (!is.na(val)) {
        p <- p + ggplot2::geom_hline(yintercept = val, linetype = "dashed",
                                     colour = "black", linewidth = 0.4)
      }
    }
    p
  ...
  # ---- Plot constructors ----
  make_trace <- function(df, y_expr, truth_vec = NULL) {
  ...
  make_density <- function(df, x_expr, truth_vec = NULL) {
  ...
  p_b <- make_density(df_mu, expression(mu), truths$mu) +
  ...
  p_d <- make_density(df_tau, expression(tau), truths$tau) +
  ...
  p_f <- make_density(df_pi, expression(pi), truths$pi) +

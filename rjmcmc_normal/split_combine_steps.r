...
split_combine <- function(lst) {
  ...
  # Pre-allocate for proposed state
  new_mu <- rep(0, new_k)
  new_tau <- rep(0, new_k)
  new_pi <- rep(0, new_k)
  new_Z <- lst$Z
  ...
  if (is_split) {
    ...
    # Randomly select a component to split
    j.star <- sample(c(1:k), size = 1)

    j1 <- j.star
    j2 <- j1 + 1

    # Copy parameters not affected by split
    if (j1 > 1) {
    ...
    # Update group assignments and counts
    for (j in c(1:new_k)) {
      new_Xv[[j]] <- X[new_Z == j]
  ...
  # Calculate acceptance probability and decide whether to accept the move
  acceptance_r <- split_combine_acceptance(
    is_split, X, ifelse(is_split, lst$k, new_k),
    Z, mu, tau, Xv, nv, pi,
